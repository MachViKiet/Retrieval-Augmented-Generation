        ...
        <Grid  size={{ xs: 12 }}>
          <Block sx = {{ 
            ...
            <ChatBlock sx = {{ 
              ...
              {History.map((conservation) => 
                (
                  <div key = {conservation?.id}>
                  {
                    conservation.role == 'bot' ?
                      <BotChat message={(conservation?.message)} sx = {{
                        background: 'linear-gradient(to right, #c7c8ff 0%, #d2ccff 100%)'
                      }}/> 
                ...
                (
                  <div key = {conservation?.id}>
                  {
                    conservation.role == 'bot' ?
                      ...
                      message={(conservation?.message)} metadata = {conservation} setOpenDetail = {setOpenDetail}/> 
                      : <UserChat message={conservation.message} metadata = {conservation}/>
                  }</div>
                )
              )}

              {<LoadingBotChat text = {stream.streamData}
                  sx = {{
                    box: {
                      background: 'linear-gradient(to right, #b9afff 0%, #ffd8d8 100%) !important'
                    }
